...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/bookbed'
db=SQLAlchemy(app)
...
# @login_required
# def pdetails():
#     email = current_user.email  # Assuming 'email' is the identifier for Hospitaluser
#     print("Email:", email)  # For debugging purposes
#     data = Bookingpatient.query.filter_by(srfid=email).first()
...
def pdetails():
    ...
    if data is None:
        flash("No booking details found.", "warning")
        return render_template("detials.html", data=data)
...
def slotbooking():
    ...
    if request.method == "POST":
        ...
        # Decrease the bed count based on the selected bed type
        if bedtype == "NormalBed":
            ...
            if seat > 0:
                ...
                flash("No available HICU beds.", "danger")
                return render_template("booking.html", query=query, query1=query1)
